...
class TDovetailJoint(Joint):
    ...
    def main_beam_ref_side_index(self):
        cross_vect = self.cross_beam.ref_sides[self.cross_beam_ref_side_index].normal.cross(
            self.main_beam.centerline.direction
        )
        ref_side_dict = beam_ref_side_incidence_with_vector(self.main_beam, self.cross_beam, cross_vect, ignore_ends=True)
        ref_side_index = min(ref_side_dict, key=ref_side_dict.get)
        return ref_side_index
